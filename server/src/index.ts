...
async function addCargoAdminUser(database: IDatabase, logger: LoggerInstance, hyperledgerConfig: IHyperledgerConfiguration, connectionManager: ComposerConnectionManager) {
  ...
  await database.passportModel.remove({ email: 'admin@quickkyc'});
...
logger.info(`yoyo data`);
  const resource = composerConnection.composerModelFactory.createMember(driverPayload);

  logger.info(`yoyo data registry`);

  const registry = await composerConnection.getRegistry(ComposerTypes.Member);

  logger.info(`yoyo data test`);

  // Check if this driver is already registered
  const exists = await registry.exists("1213");
  if (!exists) {
    registry.add(resource);
    logger.info(`added admin@quickkyc user to member registry`);
  }
}
