...
export default class DocumentController {
  ...
  /**
   * API route: create a new document
   * @param {Request} request
   * @param {ReplyNoContinue} reply
  ...
  async create(request: Hapi.Request, reply: Hapi.ReplyNoContinue): Promise<Hapi.Response> {
    ...
    try {
      ...
      if (exists) {
        ...
        return reply(Boom.badRequest(`document already exists`));
      } else {
        await registry.add(composerConnection.composerModelFactory.createDocument(payload));
        await composerConnection.disconnect();
          ...
          if (exists) {
            if (resolve) {
              return registry.resolve(id)
                .then((document) => {
                  return composerConnection.disconnect().then(() => reply(document));
                });
            } else {
              return registry.get(id)
                .then((document) => {
                  const output = composerConnection.serializeToJSON(document);
  ...
  async update(request: Hapi.Request, reply: Hapi.ReplyNoContinue): Promise<Hapi.Response> {
    ...
    try {
      ...
      if (exists) {
        ...
        composerEntityForUpdate = composerConnection.composerModelFactory.editDocument(composerEntityForUpdate, payload);
  ...
  /**
   * API route: Update the document driver with transaction
   * param {Request} request
   * param {ReplyNoContinue} reply
   * returns {Promise<Response>}
   */
   /*
//  async changedocumentDriverWithTransaction(request: Hapi.Request, reply: Hapi.ReplyNoContinue): Promise<Hapi.Response> {
  //  const identity = request.auth.credentials.id;
    //let id = request.params["id"];
    //const payload: any = request.payload;
    //try {
      //const composerConnection = await this.connectionManager.createBusinessNetworkConnection(identity);
      //const registry = await composerConnection.getRegistry(ComposerTypes.document);

      //const exists =  registry.exists(id);
      //if (exists) {
        //rajat see below line
        const transactionResourceName = composerConnection.composerModelFactory.getNamespaceForResource(ComposerTypes.ChangedocumentDriver);
